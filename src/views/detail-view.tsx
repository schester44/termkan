...
export function DetailView() {
  const { activeLane, activeCard, detailsValue, setDetailsValue, saveDetails, lanes, settings } =
    useStore();
  ...
  useInput(
    (_input, key) => {
      ...
      if (key.escape && renameMode) {
        setRenameValue('');
        setRenameMode(false);
      }
    },
    { isActive: commentMode || renameMode },
  );
  ...
  return (
    <Box flexDirection="column" padding={1}>
      ...
      <Box flexDirection="column" borderStyle="double" borderColor="cyan" paddingX={2} paddingY={1}>
        ...
        <Box marginBottom={1} gap={2}>
          <Text dimColor>Created: {new Date(currentCard.createdAt).toLocaleString()}</Text>
          <Text dimColor>Updated: {new Date(currentCard.updatedAt).toLocaleString()}</Text>
        </Box>
        ...
        <Box borderStyle="single" borderColor="yellow" paddingX={1} paddingY={0} minHeight={5}>
          <TextArea
            value={detailsValue}
            ...
            onRename={() => {
              setRenameValue(currentCard.title);
              setRenameMode(true);
            }}
          />
        </Box>
