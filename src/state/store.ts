...
import {
	loadBoard, saveBoard, loadMeta, saveMeta, listBoards, deleteBoard,
...
export const useStore = create<AppState>((set, get) => {
	...
	return {
		...
		setSettingsCursor: (val) => set((s) => ({ settingsCursor: resolveSetter(val, s.settingsCursor) })),
			...
			set({
				boardData: {
					...
					lanes: lanes.map((lane, i) => {
						if (i === activeLane) return { ...lane, cards: lane.cards.filter((_, ci) => ci !== activeCard) };
		...
		moveCardVertical: (direction) => {
			...
			[newCards[activeCard], newCards[targetIndex]] = [newCards[targetIndex]!, newCards[activeCard]!];
			set({
				boardData: {
					...boardData,
					lanes: boardData.lanes.map((lane, i) =>
