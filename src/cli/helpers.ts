...
export function resolveBoard(): { key: string; data: BoardData } {
	...
	const key = boardIdx !== -1 && globalArgs[boardIdx + 1]
		? globalArgs[boardIdx + 1]!
		: meta.lastBoard;
...
export function findCardById(data: BoardData, id: number): { card: Card; laneIndex: number; cardIndex: number } | null {
...
export function findLaneByName(data: BoardData, name: string): number {
	...
	if (idx === -1) error(`Lane "${name}" not found. Available: ${data.lanes.map((l) => l.name).join(", ")}`);
	return idx;
}
