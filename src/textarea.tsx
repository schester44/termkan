...
function styledLine(line: string): React.ReactNode {
  ...
  if (listMatch) {
    ...
    return (
      <Text>
        {indent}
        <Text color="cyan">•</Text> {styledInline(rest)}
      </Text>
    );
  }
...
export default function TextArea({
  ...
  const [vimCtx, setVimCtx] = useState<VimContext>(() => createInitialContext({ line: 0, col: 0 }));
  ...
  // Simple mode: keep cursor in bounds when value changes externally
  useEffect(() => {
    if (!vimMode) {
  ...
  useInput(
    (input, key) => {
      ...
      if (vimMode) {
        ...
        // R in normal mode renames the card title
        if (input === 'R' && vimCtx.mode === 'normal' && onRename) {
          onRename();
          return;
        }
