...
export function cmdAdd(args: string[]): void {
  ...
  if (!title)
    error(
      'Usage: tk add <title> [--lane <name>] [--details <text>] [--priority <high|medium|low|none>]',
    );
...
export function cmdMove(args: string[]): void {
  ...
  if (targetIdx === found.laneIndex) {
    out(`${dim('Already in')} ${cyan(data.lanes[targetIdx]!.name)}`, {
      ok: true,
      id,
      lane: data.lanes[targetIdx]!.name,
      moved: false,
    });
    return;
  }
...
export function cmdUpdate(args: string[]): void {
  ...
  out(`${green('✓')} Updated ${bold(`#${found.card.id}`)} "${found.card.title}"`, jsonData);
}
...
export function cmdDelete(args: string[]): void {
  ...
  out(`${green('✓')} Deleted ${bold(`#${id}`)} "${found.card.title}" from ${cyan(laneName)}`, {
    ok: true,
    id,
    title: found.card.title,
    lane: laneName,
...
export function cmdNew(args: string[]): 'open' | 'done' {
  ...
  if (detached) {
    out(`${green('✓')} Created board ${cyan(key)} ${dim('(now active)')}`, {
      ok: true,
      board: key,
    });
    return 'done';
...
export function cmdUse(args: string[]): void {
  ...
  out(`${green('✓')} Switched to board ${cyan(name)}`, { ok: true, board: name });
}
...
export function cmdPriority(args: string[]): void {
  ...
  if (!validLevels.includes(level))
    error(`Invalid priority "${level}". Use: ${validLevels.join(', ')}`);
  const id = parseInt(idStr, 10);
  if (isNaN(id)) error('Card ID must be a number');
  ...
  out(`${green('\u2713')} Set ${bold(`#${found.card.id}`)} priority to ${level}`, {
    ok: true,
    id: found.card.id,
    priority: level,
  });
...
export function cmdArchive(args: string[]): void {
  ...
  if (args.includes('--list') || args.includes('-l')) {
    ...
    if (jsonMode) {
      json({ board: key, archived: archive });
      return;
    }
    if (archive.length === 0) {
    ...
    for (const c of archive) {
      lines.push(
        `  ${dim(`#${c.id}`)} ${c.title} ${dim(`[${c.fromLane}] ${new Date(c.archivedAt).toLocaleDateString()}`)}`,
      );
    }
    process.stdout.write(lines.join('\n') + '\n');
    return;
